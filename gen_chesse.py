...
if __name__ == '__main__':
    ...
    pts1 = [[685,2027],[2913,1024],[3073,1421],[3129,1757],[3137,2329],[2945,2875]]  #target在原图片中的轮廓点
    pts2 = [[901,2169],[2825,1273],[2993,1765],[3013,2073],[2961,2553],[2861,2849]]
    pts3 = [[1057,1041],[1845,869],[2393,921],[2769,1053],[1953,3145]]
    ...
    frontimg1 = cv.imread('IMG_1928_n1.JPG')        #target原图片
    frontimg2 = cv.imread("IMG_1929_n1.JPG")
    frontimg3 = cv.imread("IMG_1930_n1.JPG")
    pts = [pts1, pts2, pts3]
    frontimgs = [frontimg1, frontimg2, frontimg3]
    for i in tqdm(range(100)):  # 生成图片(生成失败的跳过)
        # Time=time.strftime('%Y-%m-%d-%H-%M-%S')
        Time = '%06d' % i
        bgimg = cv.imread('chesse_bg.png')  # 背景图片
        h, w, _ = bgimg.shape
        ...
        for j in range(ran_tar_nums):
            ...
            #     print(i,j)
            bgimg, dst = Gen_Cheese_Datas(bgimg, frontimgs[ran_id], pts[ran_id], (ran_x, ran_y), ran_rotate,0.1)  # 生成合成图片
            shape = {}
            if dst != None:
